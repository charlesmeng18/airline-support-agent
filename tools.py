...
tools = [
    ...
    },
    {
        ...
        "function": {
            "name": "book_flight",
            "description": "Book/purchase a flight for passengers",
            "parameters": {
                "type": "object",
                "properties": {
                    "flight_number": {"type": "string", "description": "Flight number to book (e.g., AA123)"},
                    "date": {"type": "string", "description": "Flight date in YYYY-MM-DD format"},
                    "passengers": {
                        "type": "array",
                    ...
                    "class": {"type": "string", "description": "Cabin class: economy, business, first"},
                    "payment_method": {"type": "string", "description": "Payment method: credit_card, miles, points"}
                },
...
def search_one_way(origin: str, destination: str, date: str, **kwargs) -> dict:
    """Enhanced one-way search with realistic variable data"""
    # Handle both 'class' and 'class_type' parameter names
    class_type = kwargs.get('class', kwargs.get('class_type', 'economy'))
...
def search_round_trip(origin: str, destination: str, depart_date: str, return_date: str, **kwargs) -> dict:
    """Enhanced round-trip search - generates outbound and return flights"""
    # Handle both 'class' and 'class_type' parameter names
    class_type = kwargs.get('class', kwargs.get('class_type', 'economy'))
    passengers = kwargs.get('passengers', 1)
    ...
    # Generate outbound flights (origin -> destination)
    outbound_flights = []
    for i in range(random.randint(2, 4)):
        airline = random.choice(airlines)
        flight_num = f"{airline}{random.randint(100, 9999)}"
    ...
    # Generate return flights (destination -> origin)
    return_flights = []
    for i in range(random.randint(2, 4)):
        airline = random.choice(airlines)
        flight_num = f"{airline}{random.randint(100, 9999)}"
        ...
        return_flights.append({
            "flight_number": flight_num,
            "airline": airline,
            "origin": destination,  # Swapped: destination -> origin
            "destination": origin,   # Swapped: destination -> origin
            "date": return_date,
            "departure_time": f"{dep_hour:02d}:{random.randint(0, 59):02d}",
            "arrival_time": f"{int(arr_hour):02d}:{random.randint(0, 59):02d}",
            "duration": f"{int(duration_hours)}h {int((duration_hours % 1) * 60)}m",
    ...
    # Calculate total pricing for round-trip
    cheapest_outbound = min(outbound_flights, key=lambda x: x["price_usd"])
    cheapest_return = min(return_flights, key=lambda x: x["price_usd"])
    total_price = cheapest_outbound["price_usd"] + cheapest_return["price_usd"]
    
    # Optional: Add a small round-trip discount
    ...
    return {
        ...
        "pricing_summary": {
            "individual_total": round(total_price, 2),
            "round_trip_total": round(discounted_total, 2),
            "savings": round(total_price - discounted_total, 2),
            "discount_percent": round(discount * 100, 1)
        }
    }

def search_multi_city(segments: list, **kwargs) -> dict:
    """Multi-city flight search - generates flights directly"""
    # Handle both 'class' and 'class_type' parameter names  
    class_type = kwargs.get('class', kwargs.get('class_type', 'economy'))
    passengers = kwargs.get('passengers', 1)
    ...
    for i, segment in enumerate(segments):
        ...
        # Generate flight directly (same logic as search_one_way)
        airline = random.choice(airlines)
        flight_num = f"{airline}{random.randint(100, 9999)}"
...
def book_flight(flight_number: str, date: str, passengers: list, **kwargs) -> dict:
    """Book/purchase a flight - simple booking implementation"""
    class_type = kwargs.get('class', 'economy')
    payment_method = kwargs.get('payment_method', 'credit_card')
    
    # Generate booking details
    confirmation_number = f"BOOK{random.randint(100000, 999999)}"
    
    # Calculate pricing based on class and number of passengers
    base_price = random.randint(200, 800)
    if class_type == "business":
        base_price *= random.uniform(2.5, 4.0)
    elif class_type == "first":
    ...
    total_price = base_price * len(passengers)
    ...
    for i, passenger in enumerate(passengers):
        ...
        assigned_passengers.append({
            **passenger,
            ...
            "frequent_flyer": None  # Could be added later
        })
    ...
    return {
        ...
        "pricing": {
            "base_price_per_person": round(base_price, 2),
            "total_passengers": len(passengers),
            "subtotal": round(total_price, 2),
            "taxes_fees": round(total_price * 0.15, 2),  # 15% taxes/fees
            "total_amount": round(total_price * 1.15, 2)
        },
        "payment": {
        ...
        "booking_details": {
            ...
            "check_in_opens": (datetime.now() + timedelta(hours=random.randint(24, 72))).strftime("%Y-%m-%d %H:%M"),
            "baggage_allowance": "1 carry-on included, checked bags additional"
        },
        "next_steps": [
            "Check in online 24 hours before departure",
            "Arrive at airport 2 hours early for domestic flights",
            "Bring valid ID and confirmation number",
            "Download boarding passes to mobile device"
        ]
    }
...
def get_flight_details(flight_number: str, date: str = None) -> dict:
    """Detailed flight information - generates all data directly"""
    statuses = ["On Time", "Delayed", "Cancelled", "Boarding", "Departed", "Arrived", "Diverted"]
    delays = [0, 15, 30, 45, 60, 90, 120, 180]
    airports = ["JFK", "LAX", "ORD", "DFW", "ATL", "SFO", "LAS", "SEA", "MIA", "BOS"]
    aircraft_types = ["Boeing 737-800", "Airbus A320", "Boeing 777-200"]
...
def get_seat_map(flight_number: str, date: str = None, class_type: str = None) -> dict:
    """Get seat map for flight - simplified version"""
    aircraft_type = random.choice(["Boeing 737", "Airbus A320"])
    rows = random.randint(20, 35)
    
    # Simple seat generation
    available_seats = random.randint(15, 80)
    premium_seats = random.randint(5, 15)
    total_seats = rows * 6  # 6 seats per row (A-F)
    ...
    return {
        ...
        "premium_seats_available": premium_seats,
        "seat_layout": {
            "rows": rows,
            "seats_per_row": 6,
            "configuration": "3-3 (A-C, D-F)",
            "window_seats": ["A", "F"],
            "aisle_seats": ["C", "D"],
            "middle_seats": ["B", "E"]
        },
        "pricing": {
            "standard_seat": "$0-25",
            "premium_seat": "$30-75",
            "extra_legroom": "$50-150"
        },
        "availability_summary": f"{available_seats} seats available out of {total_seats} total"
    }
